...
test('pagination next and prev buttons work', async ({ page }) => {
  ...
  await expect(page.locator(':nth-match(span, 2)')).toHaveText("11");
  await expect(page.locator(':nth-match(span, 3)')).toHaveText("20");

  // Click prev button
  await page.locator('Button:text("Prev")').click();

  // check that spans have gone back to 1 to 10
  await expect(page.locator(':nth-match(span, 2)')).toHaveText("1");
  await expect(page.locator(':nth-match(span, 3)')).toHaveText("10");
})
